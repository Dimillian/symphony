...
polling:
  interval_ms: 5000
workspace:
  root: /Users/dimillian/Documents/Dev/symphony-workspaces/codexmonitor
hooks:
  after_create: |
    git clone --reference-if-able /Users/dimillian/Documents/Dev/CodexMonitor https://github.com/Dimillian/CodexMonitor.git .
agent:
...
You are working on a Linear ticket `{{ issue.identifier }}` for CodexMonitor.
...
4. Read `AGENTS.md`, `docs/codebase-map.md`, and `README.md` before changing architecture or running broad validation.
5. Use the existing validation matrix from `AGENTS.md`: always run `npm run typecheck`, run `npm run test` for frontend behavior changes, and run `cd src-tauri && cargo check` for Rust changes.
...
- Keep frontend IPC in `src/services/tauri.ts`.
- Avoid unrelated refactors in `src/App.tsx` and the listed hotspot files unless the ticket demands it.
